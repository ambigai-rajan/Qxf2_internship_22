...
# Check if the title of the page is proper
            if self.driver.title=="Wufoo!":
                print ("Success: Wufoo!")
            else:
                print ("Failed: Wufoo!")
# Find the email field using xpath with id
            name = self.driver.find_element("xpath", "//input[@id='email']")
...
# Find the password field using xpath without id
            email = self.driver.find_element("xpath", "//input[@id='password']")
            email.send_keys('123456789')

# Find the phone no field using xpath
            phone = self.driver.find_element("xpath", "//input[@id='wufooName']")
